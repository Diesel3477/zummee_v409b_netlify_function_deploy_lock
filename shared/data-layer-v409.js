...
(function(){
  'use strict';
  var VERSION='v409-server-board-photo-upload';
  ...
  function fileToDataUrl(file){return new Promise(function(resolve,reject){var reader=new FileReader();reader.onload=function(){resolve(String(reader.result||''));};reader.onerror=function(){reject(reader.error||new Error('Could not read photo file'));};reader.readAsDataURL(file);});}
  async function uploadBoardItemPhoto(row,file,opts){
    ...
    var uid=clean(opts.userId||getStoredUserId());
    if(!isUuid(uid)) return {data:null,error:new Error('Missing Zummee user id')};
    try{
      ...
      window.__lastBoardPhotoError=error;

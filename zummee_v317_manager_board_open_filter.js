/* Zummee v317 manager board open-item lifecycle filter */
(function(){
  if(window.__ZUMMEE_V317_MANAGER_BOARD_OPEN_FILTER__) return;
  window.__ZUMMEE_V317_MANAGER_BOARD_OPEN_FILTER__ = true;
  ...
  function openEvents(rows){
    ...
    rows.forEach(function(r){ if(!isConverted(r)) return; var md=meta(r); var id=s(md.board_item_id||md.source_id||md.original_board_item_id||md.board_id); if(id) converted.add(id); });
    var seen=new Set();
    return rows.filter(function(r){ if(!isSubmitted(r)) return false; var k=keyOf(r); if(!k || converted.has(k) || seen.has(k)) return false; seen.add(k); return true; });
  }
  ...
  async function fetchOpen(){ var sb=await getSB(); var cid=selectedId(); if(!sb||!cid) return []; var q=await sb.from('notification_events').select('*').eq('community_id',cid).order('created_at',{ascending:false}).limit(200); if(q.error){ console.warn('[v317 manager board] query failed', q.error); return []; } return openEvents(q.data||[]).map(norm); }
  ...
  async function refresh(){ var items=await fetchOpen(); render(items); console.log('[v317 manager board open]',items.length,'open rows'); return items; }

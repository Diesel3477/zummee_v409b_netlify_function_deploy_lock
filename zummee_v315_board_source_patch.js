/* Zummee v315 board source + Supabase fallback */
(function(){
  if(window.__ZUMMEE_V315_BOARD_SOURCE_PATCH__) return;
  window.__ZUMMEE_V315_BOARD_SOURCE_PATCH__ = true;
  ...
  function openSubmittedEvents(rows){
    ...
    rows.forEach(function(row){ if(!isConvertedEvent(row)) return; var md = getMeta(row); var id = s(md.board_item_id || md.source_id || md.original_board_item_id || md.board_id); if(id) converted.add(id); });
    var seen = new Set();
    return rows.filter(function(row){ if(!isSubmittedEvent(row)) return false; var key = rowKey(row); if(!key) return false; if(converted.has(key)) return false; if(seen.has(key)) return false; seen.add(key); return true; });
  }
  ...
  async function fetchBoardNotifications(){
    ...
    if(res && res.error){ console.warn('[v317] notification_events failed', res.error); return []; }
